...
export const getPosts = async (req, res) => {
  ...
  try {
    ...
    const posts = await PostMessage.find().sort({ _id: -1 }).limit(LIMIT).skip(startIndex); //sort newest first
...
export const updatePost = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(_id)) return res.status(404).send("No post of that id");
